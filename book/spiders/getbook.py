...
class GetbookSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            yield scrapy.Request(self.prefix + node.xpath("./h3/a/@href").extract()[0], meta={'item': item},
                                 callback=self.go)
    ...
    def go(self, response):
        yield scrapy.Request("https://" + response.xpath("//a[@class = 'pink-btn J-getJumpUrl ']/@href").extract()[0],
                             meta={'item': response.meta['item']}, callback=self.getContent)
    ...
    def getContent(self, response):
        ...
        item['book_chapter_content'] = \
        response.xpath("normalize-space(string(//div[@class='main-text-wrap']/div[2]))").extract()[0].replace(u'\u3000',
                                                                                                              '')

        yield item

        if response.xpath("//a[@id='j_chapterNext']/text()").extract()[0] != "书末页":
            yield scrapy.Request("https://" + response.xpath("//a[@id='j_chapterNext']/@href").extract()[0],
                                 meta={'item': item}, callback=self.getContent)
